    ...
    <v-card>
      <v-card-title>
        <div class="d-flex justify-space-between align-center">
          <span class="text-h5">Риски ребенка</span>
          <v-btn icon="mdi-arrow-left" variant="text" @click="goBack" />
        </div>
      </v-card-title>
      <v-card-text>
        <div v-if="childInfo" class="mb-4">
...
const childInfo = ref<{ id: number; fio: string } | null>(null)
...
const loadChildInfo = async () => {
  try {
    const user = await usersApi.get(childId)
    childInfo.value = {
      id: user.id,
      fio: user.fio,
    }
...
const loadRisks = async () => {
  loading.value = true
  try {
    const response = await analyticsApi.getMyRisks({ child_id: childId })
